...
// NOTA: Estas funciones son necesarias aquí porque el componente es de cliente
// y Next.js no puede importar funciones de un componente de servidor directamente.
...
export default function FeedDisplay({ initialLists }: FeedDisplayProps) {
    const searchParams = useSearchParams();
    const activeTab: Tab = (searchParams.get('tab') as Tab) || 'destacados';
    
    // Selecciona qué lista mostrar en el feed principal
    const dealsToShow = initialLists[activeTab] || initialLists.destacados;

    // Los datos del sidebar no cambian, por lo que usamos las listas pre-cargadas
    const { topToday, topWeek } = initialLists;
    
    // Función para renderizar una tarjeta de chollo (misma que ya tenías)
    const renderDealCard = (deal: any) => {
        ...
        return (
            <div key={deal.id} className="bg-[#222529] rounded-lg border border-gray-700/50 overflow-hidden flex flex-col md:flex-row group relative shadow-md">
                {/* ... (Todo el JSX de la tarjeta, incluyendo VoteControl) ... */}
                ...
                <div className="flex-1 p-4 flex flex-col relative z-0">
                  ...
                  <div className="mt-auto flex justify-end items-center pt-3 border-t border-gray-700">
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              {dealsToShow.map(renderDealCard)}
            </div>
